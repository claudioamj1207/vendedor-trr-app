...
import { analisarLinhasClientes, classificarCliente, formatarDocumento } from '../lib/clientesViva.mjs';
test('ERP: CPF, CNPJ, leading zeros, incomplete document and addresses',{skip:!process.env.VIVA_CSV_PATH},()=>{
 ...
 assert.equal(r.registros.length,868);assert.equal(r.pendencias.length,2);
 assert.equal(r.registros.filter(x=>x.documento.length===11).length,228);
...
test('all UFs and formatted CPF; exact branch match; unavailable base',()=>{
 ...
 assert.equal(r.registros.length,3);assert.equal(r.registros[0].uf,'SP');assert.equal(r.pendencias.length,1);
...
test('duplicates consolidate and unrecognized input never becomes an empty replacement',()=>{
 const r=analisarLinhasClientes([['Documento','Nome'],['01234567890','Um'],['01234567890','Dois']]);
