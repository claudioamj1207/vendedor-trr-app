...
export const documentoCompleto = value => /^(\d{11}|\d{14})$/.test(normalizarDocumento(value));
...
export function analisarLinhasClientes(rows) {
  ...
  const adicionar = (documento, nome, extra, linha) => {
    ...
    if (!documentoCompleto(d) || !String(nome || '').trim()) {
      pendencias.push({ linha, documento: String(documento ?? ''), nome: String(nome ?? ''), motivo: 'Informe CPF com 11 ou CNPJ com 14 dígitos e nome. Zeros não são completados automaticamente.' });
      atual = null; return;
    }
  ...
  return { registros: [...registros.values()], pendencias, duplicados };
}
