export async function consultarCNPJNaBrasilAPI(cnpj) {
    ...
    for (let tentativa = 1; tentativa <= MAX_TENTATIVAS; tentativa++) {
      try {
        ...
        if (!res.ok) {
          ...
          return { ok: true, dados: mapearBrasilAPI(info) };
        }
      } catch (err) {
        if (tentativa === MAX_TENTATIVAS) {
  ...
  const consultarCNPJws = async () => {
    try {
      ...
      if (!info?.estabelecimento?.cnpj && !info?.razao_social) {
        return { ok: false, erro: `Resposta inválida da CNPJ.ws para ${cnpjLimpo}` };
      }

      return { ok: true, dados: mapearCNPJws(info) };
    } catch (err) {
      return { ok: false, erro: 'Falha de conexão com a CNPJ.ws' };
    }
  };
  ...
  if (brasil.ok && complementar.ok) {
    return {
      ok: true,
      dados: mesclarDados(brasil.dados, complementar.dados),
  ...
  if (brasil.ok) {
    return {
      ...
      dados: brasil.dados,
      fontes: ['BrasilAPI'],
      aviso: complementar.erro || 'Fonte complementar indisponível.',
    };
  }
  ...
  if (complementar.ok) {
    return {
      ok: true,
      dados: complementar.dados,
      fontes: ['CNPJ.ws'],
      aviso: brasil.erro || 'BrasilAPI indisponível.',
    };
  }
  ...
  return {
    ...
    detalhes: [brasil.erro, complementar.erro].filter(Boolean),
  };
}
