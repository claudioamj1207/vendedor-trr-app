export async function consultarCNPJNaBrasilAPI(cnpj) {
  ...
  const mapearBrasilAPI = (info = {}) => {
    return {
      ...
      nome_fantasia: normalizarTexto(info.nome_fantasia, normalizarTexto(info.razao_social)),
      ...
      cnaes_secundarios: Array.isArray(info.cnaes_secundarios) ? info.cnaes_secundarios : [],
      descricao_situacao_cadastral: normalizarTexto(info.descricao_situacao_cadastral, 'ATIVA'),
  ...
  const mapearCNPJws = (info = {}) => {
    ...
    const atividadesSecundarias = Array.isArray(estabelecimento.atividades_secundarias)
    ...
    return {
      cnpj: cnpjLimpo,
      razao_social: normalizarTexto(info.razao_social),
      nome_fantasia: normalizarTexto(estabelecimento.nome_fantasia, normalizarTexto(info.razao_social)),
      ...
      descricao_situacao_cadastral: normalizarTexto(estabelecimento.situacao_cadastral, 'ATIVA'),
      capital_social: info.capital_social ?? '',
      porte: normalizarTexto(info.porte?.descricao),
      data_inicio_atividade: normalizarTexto(estabelecimento.data_inicio_atividade),
  ...
  const mesclarDados = (principal = {}, complementar = {}) => {
    ...
    return {
      ...
      nome_fantasia: escolher(principal.nome_fantasia, complementar.nome_fantasia),
      ...
      cnaes_secundarios:
        secundariasPrincipal.length > 0 ? secundariasPrincipal : secundariasComplementar,
      descricao_situacao_cadastral: escolher(
        principal.descricao_situacao_cadastral,
        complementar.descricao_situacao_cadastral
      ),
      capital_social: escolher(principal.capital_social, complementar.capital_social),
  ...
  let dadosFinais = brasil.dados;

  if (precisaComplementoEndereco(brasil.dados)) {
    await sleep(800);
    const complementar = await consultarCNPJws();

    if (complementar.ok) {
      dadosFinais = mesclarDados(brasil.dados, complementar.dados);
    }
  }
