          ...
          {leads.map(lead => (
            <div key={lead.cnpj} className="bg-zinc-900/50 p-4 rounded-3xl border border-white/5">
              ...
              <div className="grid grid-cols-4 gap-2 mt-4">
                ...
                <button onClick={() => window.print()} className="h-9 bg-zinc-800 rounded-xl flex items-center justify-center text-[10px]">📝</button>
                <button onClick={async () => { await supabase.from('empresas_mestre').update({status_lead: aba === 'estoque' ? 'Triagem' : 'Em Prospecção'}).eq('cnpj', lead.cnpj); sincronizar(); }} className="h-9 bg-blue-600 rounded-xl flex items-center justify-center text-[10px]">➡️</button>
                <button onClick={async () => { await supabase.from('empresas_mestre').update({status_lead: 'Viável'}).eq('cnpj', lead.cnpj); sincronizar(); }} className="h-9 bg-white text-black font-black rounded-xl flex items-center justify-center text-[10px]">✅</button>
              </div>
            </div>
          ))}
        </div>
