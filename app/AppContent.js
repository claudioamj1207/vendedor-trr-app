...
export default function VendedorTRR_Master() {
  ...
  const leadsFiltrados = useMemo(() => {
    return leads.filter(lead => {
      ...
      return matchRazao && matchFantasia && matchCNPJ && matchBairro && matchFonte && matchCnae && matchMuni && matchData && matchBusca;
    });
        ...
        <div className="flex justify-between items-center">
          ...
          {moduloAtivo === 'todo' && (
            <button onClick={() => setMostrarFiltros(!mostrarFiltros)} className="text-[10px] bg-blue-600/20 text-blue-400 px-4 py-2 rounded-full font-bold border border-blue-500/30 active:scale-95 transition-all">
              {mostrarFiltros ? 'FECHAR FILTROS' : 'FILTROS ESTILO EXCEL'}
            </button>
          )}
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-4 bg-zinc-900/50 rounded-2xl border border-white/5 animate-in slide-in-from-top-4">
                {[
                  ...
                  <div key={filtro.campo} className="flex flex-col gap-1">
                    ...
                    <select 
                      ...
                      className="bg-zinc-800 text-[11px] p-2.5 rounded-lg outline-none border border-white/5 appearance-none text-white"
                    >
                      {obterOpcoes(filtro.campo).map(opt => (
                        <option key={opt} value={opt}>{opt === 'Todos' ? `Todos (${filtro.label})` : opt}</option>
                      ))}
                    </select>
                  </div>
                ))}
                <div className="flex flex-col gap-1">
                  ...
                  <select 
                    ...
                    className="bg-zinc-800 text-[11px] p-2.5 rounded-lg border border-white/5 text-white"
                  >
                ...
                <div key={lead.cnpj} className="py-4 px-4 flex justify-between items-center gap-3 hover:bg-zinc-800/50 transition-colors">
                  ...
                  <div className="shrink-0">
                    <button onClick={async () => { 
                      ...
                      sincronizar(); 
                    }} className={`h-10 w-12 rounded-xl flex items-center justify-center text-sm active:scale-90 transition-all ${aba === 'estoque' ? 'bg-blue-600' : 'bg-orange-600'}`}>
                      ➡️
                    </button>
                  </div>
                </div>
