"use client"; import React, { useState, useEffect, useMemo, useCallback } from 'react'; import { supabase } from '../lib/supabase'; import { consultarCNPJNaBrasilAPI } from '../lib/brasilApi'; import * as XLSX from 'xlsx';

const STATUS_LEAD = { NOVO: 'Novo', TRIAGEM: 'Triagem', EM_PROSPECCAO: 'Em Prospecção' };

const MODULOS = { TODO: 'todo', PESCARIA: 'pescaria' };

const ABAS = { ESTOQUE: 'estoque', TRIAGEM: 'triagem' };

const FILTROS_INICIAIS = { razao_social: 'Todos', nome_fantasia: 'Todos', cnpj: 'Todos', bairro: 'Todos', fonte_lead: 'Todos', cnae_principal_descricao: 'Todos', cnae_secundario: 'Todos' };

const CAMPOS_FILTRO = [ { label: 'Razão Social', campo: 'razao_social' }, { label: 'Nome Fantasia', campo: 'nome_fantasia' }, { label: 'CNPJ', campo: 'cnpj' }, { label: 'Bairro', campo: 'bairro' }, { label: 'Fonte', campo: 'fonte_lead' }, { label: 'CNAE Principal', campo: 'cnae_principal_descricao' } ];

const CAMPOS_COM_BUSCA_EXATA = [ 'razao_social', 'nome_fantasia', 'cnpj', 'bairro', 'fonte_lead', 'cnae_principal_descricao' ];

const CAMPOS_COMPARACAO_POR_INCLUDES = ['cnae_secundario'];

const ITENS_POR_PAGINA = 50;

const normalizarCNPJ = (cnpj) => String(cnpj || '').replace(/\D/g, '');

const formatarCNPJ = (cnpj) => { const limpo = normalizarCNPJ(cnpj); if (limpo.length !== 14) return cnpj; return limpo.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})$/, '$1.$2.$3/$4-$5'); };

const extrairCNPJsDoTexto = (texto) => { if (!texto) return [];

const regex = /\d{2}[.\s,/-]?\d{3}[.\s,/-]?\d{3}[/\s-]?\d{4}[-\s]?\d{2}|\d{14}/g; const encontrados = texto.match(regex) || [];

return [ ...new Set( encontrados .map((item) => normalizarCNPJ(item)) .filter((cnpj) => cnpj.length === 14) ) ]; };

const gerarIndiceBusca = (lead) => { return Object.entries(lead) .filter(([chave]) => chave !== '_busca') .map(([, valor]) => String(valor || '').toLowerCase()) .join(' '); };

const enriquecerLeadParaBusca = (lead) => { const leadBase = { ...lead, cnpj_normalizado: normalizarCNPJ(lead.cnpj) };

return { ...leadBase, _busca: gerarIndiceBusca(leadBase) }; };

const lerArquivoComoTexto = async (file) => { return new Promise((resolve, reject) => { const reader = new FileReader();
...
export default function VendedorTRR_Master() { const [leads, setLeads] = useState([]); const [aba, setAba] = useState(ABAS.ESTOQUE); const [moduloAtivo, setModuloAtivo] = useState(MODULOS.TODO); const [buscaGlobal, setBuscaGlobal] = useState(''); const [cnpjBusca, setCnpjBusca] = useState(''); const [carregando, setCarregando] = useState(true); const [statusProcesso, setStatusProcesso] = useState(''); const [resultadoBusca, setResultadoBusca] = useState(''); const [erroBusca, setErroBusca] = useState(''); const [mostrarFiltros, setMostrarFiltros] = useState(false); const [mostrarExportacao, setMostrarExportacao] = useState(false); const [exportandoBackup, setExportandoBackup] = useState(false); const [exportandoFiltrados, setExportandoFiltrados] = useState(false); const [totalAbsoluto, setTotalAbsoluto] = useState(0); const [ultimosCnpjsProcessados, setUltimosCnpjsProcessados] = useState([]); const [ultimosCnpjsFalhados, setUltimosCnpjsFalhados] = useState([]); const [paginaAtual, setPaginaAtual] = useState(1); const [filtrosAtivos, setFiltrosAtivos] = useState(FILTROS_INICIAIS); const [buscaDebounced, setBuscaDebounced] = useState('');

const limparMensagens = useCallback(() => { setResultadoBusca(''); setErroBusca(''); }, []);

const resetarEstadoPescaria = useCallback(() => { setStatusProcesso(''); setUltimosCnpjsProcessados([]); setUltimosCnpjsFalhados([]); }, []);

const atualizarFiltro = useCallback((campo, valor) => { setFiltrosAtivos((prev) => ({ ...prev, [campo]: valor })); }, []);

const limparFiltros = useCallback(() => { setFiltrosAtivos(FILTROS_INICIAIS); }, []);

const prepararDadosPlanilha = useCallback((dados) => { return dados.map((lead) => ({ 'Razão Social': lead.razao_social || '', 'Nome Fantasia': lead.nome_fantasia || '', 'CNPJ': formatarCNPJ(lead.cnpj || ''), 'CNPJ Limpo': normalizarCNPJ(lead.cnpj || ''), 'Logradouro': lead.logradouro || '', 'Número': lead.numero || '', 'Bairro': lead.bairro || '', 'Município': lead.municipio || '', 'UF': lead.uf || '', 'Fonte do Lead': lead.fonte_lead || '', 'CNAE Principal': lead.cnae_principal_descricao || '', 'CNAE Secundário': lead.cnae_secundario || '', 'Situação Cadastral': lead.situacao_cadastral || '', 'Status do Lead': lead.status_lead || '' })); }, []);

const buscarTodosDoBanco = useCallback(async () => { let todos = []; let de = 0; let ate = 999; let continua = true;
...
const exportarBackupBancoCompleto = useCallback(async () => { try { setExportandoBackup(true); limparMensagens();
...
const leadsFiltrados = useMemo(() => { return leads.filter((lead) => { if (buscaDebounced && !(lead._busca && lead._busca.includes(buscaDebounced))) { return false; }
...
const exportarFiltrados = useCallback(async () => { try { setExportandoFiltrados(true); limparMensagens();
...
const processarEmLotes = useCallback(async ({ itens, tamanhoLote = 5, pausaMs = 300, mensagemProgresso, processador }) => { let sucesso = 0; let ultimoErro = ''; const falhados = [];
...
const processarCNPJ = useCallback(async (cnpj, leadExistente = {}) => { const cnpjLimpo = normalizarCNPJ(cnpj);
...
const sincronizar = useCallback(async () => { try { setCarregando(true);
...
useEffect(() => { const timeout = setTimeout(() => { setBuscaDebounced(buscaGlobal.trim().toLowerCase()); }, 250);

return () => clearTimeout(timeout);
}, [buscaGlobal]);

const opcoesFiltros = useMemo(() => { const opcoes = {};
...
const totalPaginas = useMemo(() => Math.max(1, Math.ceil(leadsFiltrados.length / ITENS_POR_PAGINA)), [leadsFiltrados.length]);

const leadsPaginados = useMemo(() => { const inicio = (paginaAtual - 1) * ITENS_POR_PAGINA; const fim = inicio + ITENS_POR_PAGINA; return leadsFiltrados.slice(inicio, fim); }, [leadsFiltrados, paginaAtual]);

const paginaInicial = useMemo(() => { if (leadsFiltrados.length === 0) return 0; return (paginaAtual - 1) * ITENS_POR_PAGINA + 1; }, [paginaAtual, leadsFiltrados.length]);

const paginaFinal = useMemo(() => Math.min(paginaAtual * ITENS_POR_PAGINA, leadsFiltrados.length), [paginaAtual, leadsFiltrados.length]);

useEffect(() => { setPaginaAtual(1); }, [buscaDebounced, filtrosAtivos, aba, moduloAtivo]);

useEffect(() => { if (paginaAtual > totalPaginas) { setPaginaAtual(totalPaginas); } }, [paginaAtual, totalPaginas]);

const moverLead = useCallback(async (lead) => { await supabase .from('empresas_mestre') .update({ status_lead: aba === ABAS.ESTOQUE ? STATUS_LEAD.TRIAGEM : STATUS_LEAD.EM_PROSPECCAO }) .eq('cnpj', lead.cnpj);

await sincronizar();
}, [aba, sincronizar]);

const limparInativos = useCallback(async () => { if (!confirm(Limpar inativos e duplicados? Isso vai verificar ${leadsFiltrados.length} lead(s) da tela e depois limpar duplicidades no banco.)) { return; }
...
const atualizarFaltantes = useCallback(async () => { limparMensagens();
...
const extrairEPesquisar = useCallback(async (e) => { const file = e.target.files?.[0]; if (!file) return;
...
const buscarECadastrarCNPJs = useCallback(async () => { limparMensagens(); setUltimosCnpjsFalhados([]);
...
const trocarModulo = useCallback((modulo) => { setModuloAtivo(modulo); limparMensagens(); resetarEstadoPescaria(); }, [limparMensagens, resetarEstadoPescaria]);

const irParaPagina = useCallback((pagina) => { if (pagina < 1 || pagina > totalPaginas) return; setPaginaAtual(pagina); }, [totalPaginas]);

return (

Vendedor TRR
{[MODULOS.TODO, MODULOS.PESCARIA].map((m) => ( <button key={m} onClick={() => trocarModulo(m)} className={moduloAtivo === m ? 'text-white border-b border-blue-500' : 'text-zinc-600'} > {m === MODULOS.TODO ? 'LISTA' : 'PESCARIA DE CNPJ'} ))}
    <div className="flex justify-between items-center gap-3">
      <h2 className="text-3xl font-black italic uppercase tracking-tighter text-white">
        {moduloAtivo === MODULOS.TODO ? (aba === ABAS.TRIAGEM ? 'Triagem' : 'Estoque') : 'Pescaria de CNPJ'}
      </h2>
      ...
      <div className="flex gap-2 flex-wrap justify-end">
        {moduloAtivo === MODULOS.TODO && (
          <>
            ...
            <button onClick={() => setMostrarExportacao(!mostrarExportacao)} className="text-[9px] bg-blue-700 px-4 py-2 rounded-full font-bold border border-blue-400/20">EXPORTAR XLSX</button>
          </>
        )}
      ...
      <div className="mt-4 space-y-3">
        ...
        {mostrarExportacao && (
          <div className="p-4 bg-zinc-900 rounded-2xl border border-blue-500/10 space-y-4">
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-blue-400 mb-2">Exportação XLSX</p>
              <p className="text-[11px] text-zinc-400">Você pode gerar um backup do banco inteiro ou exportar todos os leads filtrados da tela atual.</p>
            </div>
            <div className="flex gap-2 flex-wrap">
              <button onClick={exportarBackupBancoCompleto} disabled={exportandoBackup || exportandoFiltrados} className="text-[10px] bg-emerald-600 px-4 py-2 rounded-full font-bold disabled:opacity-50">{exportandoBackup ? 'GERANDO BACKUP...' : 'BACKUP DO BANCO'}</button>
              <button onClick={exportarFiltrados} disabled={exportandoBackup || exportandoFiltrados} className="text-[10px] bg-blue-600 px-4 py-2 rounded-full font-bold disabled:opacity-50">{exportandoFiltrados ? 'EXPORTANDO FILTRADOS...' : 'EXPORTAR FILTRADOS'}</button>
              <button onClick={() => setMostrarExportacao(false)} disabled={exportandoBackup || exportandoFiltrados} className="text-[10px] bg-zinc-800 px-4 py-2 rounded-full font-bold border border-white/10 disabled:opacity-50">FECHAR</button>
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 p-4 bg-zinc-900 rounded-2xl border border-white/5">
            {CAMPOS_FILTRO.map((filtro) => (
              <div key={filtro.campo} className="flex flex-col gap-1">
                ...
                <select value={filtrosAtivos[filtro.campo]} onChange={(e) => atualizarFiltro(filtro.campo, e.target.value)} className="bg-zinc-800 text-[11px] p-2.5 rounded-lg text-white outline-none">
            ...
            <button onClick={limparFiltros} className="lg:col-span-3 text-[9px] font-bold text-red-500 uppercase py-2 bg-red-500/10 rounded-lg">Limpar Filtros</button>
          </div>
        )}

        <div className="flex justify-between items-center px-1 gap-3 flex-wrap">
          <div className="flex gap-4 items-center flex-wrap">
            <p className="text-[10px] text-zinc-400 font-black uppercase tracking-widest">{totalAbsoluto} CNPJs no banco</p>
            <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest">{leadsFiltrados.length} na tela</p>
            {!carregando && buscaGlobal !== buscaDebounced && <p className="text-[10px] text-blue-500 font-black uppercase tracking-widest animate-pulse">Atualizando busca...</p>}
            {!carregando && leadsFiltrados.length > 0 && <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest">Mostrando {paginaInicial}-{paginaFinal}</p>}
          </div>
    ...
    {moduloAtivo === MODULOS.TODO && (
      <>
        <div className="bg-zinc-900/30 border border-white/5 rounded-2xl divide-y divide-zinc-800/50">
          {carregando ? (
            <div className="text-center py-20 text-[10px] animate-pulse text-zinc-600 font-black uppercase tracking-widest">Sincronizando...</div>
          ) : leadsFiltrados.length === 0 ? (
            <div className="text-center py-20 text-[10px] text-zinc-600 font-black uppercase tracking-widest">Nenhum lead encontrado</div>
          ) : (
            leadsPaginados.map((lead) => (
              <div key={lead.cnpj} className="py-4 px-4 flex justify-between items-center gap-3 hover:bg-zinc-800/40 transition-colors">
                ...
                <button onClick={() => moverLead(lead)} className="h-10 w-10 bg-blue-600 rounded-xl flex items-center justify-center text-white active:scale-95 transition-all">➡️</button>
              </div>
            ))
          )}
        </div>
        ...
        {!carregando && leadsFiltrados.length > 0 && totalPaginas > 1 && (
          <div className="mt-6 bg-zinc-900/50 border border-white/5 rounded-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="text-[10px] text-zinc-400 font-black uppercase tracking-widest">Página {paginaAtual} de {totalPaginas}</div>
            <div className="flex items-center gap-2 flex-wrap justify-center">
              <button onClick={() => irParaPagina(1)} disabled={paginaAtual === 1} className="px-3 py-2 rounded-xl text-[10px] font-black bg-zinc-800 border border-white/10 text-white disabled:opacity-30">Primeira</button>
              <button onClick={() => irParaPagina(paginaAtual - 1)} disabled={paginaAtual === 1} className="px-3 py-2 rounded-xl text-[10px] font-black bg-zinc-800 border border-white/10 text-white disabled:opacity-30">Anterior</button>
              <button onClick={() => irParaPagina(paginaAtual + 1)} disabled={paginaAtual === totalPaginas} className="px-3 py-2 rounded-xl text-[10px] font-black bg-zinc-800 border border-white/10 text-white disabled:opacity-30">Próxima</button>
              <button onClick={() => irParaPagina(totalPaginas)} disabled={paginaAtual === totalPaginas} className="px-3 py-2 rounded-xl text-[10px] font-black bg-zinc-800 border border-white/10 text-white disabled:opacity-30">Última</button>
            </div>
          </div>
        )}
      </>
    ...
    {moduloAtivo === MODULOS.PESCARIA && (
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="bg-zinc-900 p-8 rounded-3xl border border-dashed border-zinc-800 text-center">
          ...
          <p className="text-[11px] text-zinc-500 mb-4">Envie Excel (.xlsx), texto (.txt) ou PDF textual para pescar CNPJs.</p>
        ...
        <div className="max-w-3xl mx-auto space-y-4 text-white">
          ...
          <p className="text-[11px] text-zinc-500">Cole CNPJs com ou sem máscara, ou até texto misturado.</p>
          <textarea placeholder="Cole os CNPJs aqui..." className="w-full bg-zinc-900 p-4 rounded-2xl text-sm h-40 outline-none border border-zinc-800 text-white" value={cnpjBusca} onChange={(e) => setCnpjBusca(e.target.value)} />
          <button onClick={buscarECadastrarCNPJs} className="w-full bg-blue-600 py-4 rounded-2xl font-black uppercase text-sm text-white shadow-lg active:scale-95 transition-all">PESCAR E SALVAR</button>
        </div>
        ...
        {ultimosCnpjsProcessados.length > 0 && (
          <div className="bg-zinc-900/70 border border-white/5 rounded-3xl p-6">
            <div className="flex justify-between items-center mb-4 gap-4">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400">{ultimosCnpjsProcessados.length} CNPJ(s)</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {ultimosCnpjsProcessados.map((cnpj) => (
                <span key={cnpj} className="text-[10px] bg-blue-900/20 px-3 py-2 rounded-full text-blue-300 font-bold border border-blue-500/10">{formatarCNPJ(cnpj)}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {ultimosCnpjsFalhados.length > 0 && (
          <div className="bg-red-900/20 border border-red-500/20 rounded-3xl p-6">
            <div className="flex justify-between items-center mb-4 gap-4">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-red-200">{ultimosCnpjsFalhados.length} falha(s)</span>
            </div>
            <div className="space-y-2">
              {ultimosCnpjsFalhados.map((item) => (
                <div key={`${item.cnpj}-${item.erro}`} className="bg-black/20 border border-red-500/10 rounded-2xl px-4 py-3">
                  <p className="text-[11px] font-black text-red-200">{formatarCNPJ(item.cnpj)}</p>
  ...
  <nav className="fixed bottom-6 left-6 right-6 h-16 bg-zinc-900/90 backdrop-blur-md border border-white/10 rounded-full px-8 flex justify-around items-center z-50 shadow-2xl">
    {[ABAS.ESTOQUE, ABAS.TRIAGEM].map((a) => (
      <button key={a} onClick={() => setAba(a)} className={`text-[11px] font-black uppercase tracking-widest ${aba === a ? 'text-blue-500' : 'text-zinc-600'}`}>
        {a}
      </button>
    ))}
  </nav>
